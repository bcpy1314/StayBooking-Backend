...
public interface StayAvailabilityRepository extends JpaRepository<StayAvailability, StayAvailabilityKey> {
    // select stay-id FROM stayAvailability WHERE stay_id in ids AND state = 0 AND data between startdate and enddate GROUPBY stay_id having count = 4
    // duration is to check if the return value meet the requirement, since the query will return all value meets the data requirement
    ...
    @Modifying// is to tell Jpa this is an update or written operation
